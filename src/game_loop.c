...
/*   Updated: 2025/02/27 13:45:23 by secros           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void movement(t_data *data)
{
	...
	{
		if (check_collision(data, data->player.hitbox, (t_vect) {small_move.x, 0}, '1'))
		{
		...
		if (check_collision(data, data->player.hitbox, (t_vect) {0, small_move.y}, '1'))
		{
...
{
	if (data->engine.end == 0 && delta_time(data, &data->engine.time, FRAME_RATE))
	{
		movement(data);
		step_count(data);
		world_init(data);
